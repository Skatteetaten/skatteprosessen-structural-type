        ...
        if (implementationGenerations.contains(ImplementationGeneration.PROJECTION)) {
            ...
            if (featureGenerations.contains(FeatureGeneration.FACTORY_ON_STRUCTURE)) {
                ...
                compound.getSingulars().forEach(singular -> {
                    CodeBlock.Builder of = CodeBlock.builder();
                    compound.getSubDescriptions().forEach(compoundSubtype -> singular.getSubDescriptions().stream()
                        .filter(compoundSubtype.getSingulars()::contains)
                        .forEach(singluarSubtype -> of.beginControlFlow(
                            "if (value instanceof $T)",
                            singluarSubtype.getType()
                        ).addStatement(
                            "return $T.of(($T) value)",
                            nameResolver.structure(compoundSubtype),
                            singluarSubtype.getType()
                        ).endControlFlow()));
